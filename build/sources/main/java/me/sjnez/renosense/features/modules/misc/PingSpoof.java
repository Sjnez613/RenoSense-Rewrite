...
public class PingSpoof
    ...
    public void onPacketSend(PacketEvent.Send event) {
        if (this.receive && PingSpoof.mc.player != null && !Util.mc.isSingleplayer() && PingSpoof.mc.player.isEntityAlive() && event.getStage() == 0 && event.getPacket() instanceof CPacketKeepAlive) {
    ...
    public void clearQueue() {
        if (PingSpoof.mc.player != null && !Util.mc.isSingleplayer() && PingSpoof.mc.player.isEntityAlive() && (!this.seconds.getValue().booleanValue() && this.timer.passedMs(this.delay.getValue().intValue()) || this.seconds.getValue().booleanValue() && this.timer.passedS(this.secondDelay.getValue().intValue()))) {
