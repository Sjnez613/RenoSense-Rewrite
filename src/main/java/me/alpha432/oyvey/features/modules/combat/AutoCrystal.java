...
public class AutoCrystal
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (!this.justRender.getValue().booleanValue() && this.switchTimer.passedMs(this.switchCooldown.getValue().intValue()) && this.explode.getValue().booleanValue() && this.instant.getValue().booleanValue() && event.getPacket() instanceof SPacketSpawnObject && (this.syncedCrystalPos == null || !this.syncedFeetPlace.getValue().booleanValue() || this.damageSync.getValue() == DamageSync.NONE)) {
            ...
            if (packet2.getType() == 51 && AutoCrystal.mc.player.getDistanceSq(pos = new BlockPos(packet2.getX(), packet2.getY(), packet2.getZ())) + (double) this.predictOffset.getValue().floatValue() <= MathUtill.square(this.breakRange.getValue().floatValue()) && (this.instantTimer.getValue() == PredictTimer.NONE || this.instantTimer.getValue() == PredictTimer.BREAK && this.breakTimer.passedMs(this.breakDelay.getValue().intValue()) || this.instantTimer.getValue() == PredictTimer.PREDICT && this.predictTimer.passedMs(this.predictDelay.getValue().intValue()))) {
                ...
                if (this.predictFriendDmg.getValue().booleanValue() && (this.antiFriendPop.getValue() == AntiFriendPop.BREAK || this.antiFriendPop.getValue() == AntiFriendPop.ALL) && this.isRightThread()) {
                    for (EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        if (friend == null || AutoCrystal.mc.player.equals(friend) || friend.getDistanceSq(pos) > MathUtill.square(this.range.getValue().floatValue() + this.placeRange.getValue().floatValue()) || !OyVey.friendManager.isFriend(friend) || !((double) DamageUtill.calculateDamage(pos, friend) > (double) EntityUtill.getHealth(friend) + 0.5))
                            continue;
                        return;
                    }
                }
                if (placedPos.contains(pos.down())) {
                    float selfDamage;
                    if (this.isRightThread() && this.superSafe.getValue() != false ? DamageUtill.canTakeDamage(this.suicide.getValue()) && ((double) (selfDamage = DamageUtill.calculateDamage(pos, AutoCrystal.mc.player)) - 0.5 > (double) EntityUtill.getHealth(AutoCrystal.mc.player) || selfDamage > this.maxSelfBreak.getValue().floatValue()) : this.superSafe.getValue() != false) {
                        return;
                    }
                    ...
                    if (DamageUtill.canTakeDamage(this.suicide.getValue())) {
                        selfDamage = DamageUtill.calculateDamage(pos, AutoCrystal.mc.player);
                    }
                    if ((double) selfDamage + 0.5 < (double) EntityUtill.getHealth(AutoCrystal.mc.player) && selfDamage <= this.maxSelfBreak.getValue().floatValue()) {
                        for (EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                            float damage;
                            if (!(player.getDistanceSq(pos) <= MathUtill.square(this.range.getValue().floatValue())) || !EntityUtill.isValid(player, this.range.getValue().floatValue() + this.breakRange.getValue().floatValue()) || this.antiNaked.getValue().booleanValue() && DamageUtill.isNaked(player) || !((damage = DamageUtill.calculateDamage(pos, player)) > selfDamage || damage > this.minDamage.getValue().floatValue() && !DamageUtill.canTakeDamage(this.suicide.getValue())) && !(damage > EntityUtill.getHealth(player)))
                                continue;
            ...
            if (this.threadMode.getValue() == ThreadMode.SOUND && this.isRightThread() && AutoCrystal.mc.player != null && AutoCrystal.mc.player.getDistanceSq(pos) < MathUtill.square(this.soundPlayer.getValue().floatValue())) {
    ...
    public void onRender3D(Render3DEvent event) {
        if ((this.offHand || this.mainHand || this.switchMode.getValue() == Switch.CALC) && this.renderPos != null && this.render.getValue().booleanValue() && (this.box.getValue().booleanValue() || this.text.getValue().booleanValue() || this.outline.getValue().booleanValue())) {
            RenderUtil.drawBoxESP(this.renderPos, this.colorSync.getValue() != false ? ColorUtil.rainbow((int) HUD.getInstance().rainbowHue.getValue()) : new Color(this.red.getValue(), this.green.getValue(), this.blue.getValue(), this.alpha.getValue()), this.customOutline.getValue(), this.colorSync.getValue() != false ? getCurrentColor() : new Color(this.cRed.getValue(), this.cGreen.getValue(), this.cBlue.getValue(), this.cAlpha.getValue()), this.lineWidth.getValue().floatValue(), this.outline.getValue(), this.box.getValue(), this.boxAlpha.getValue(), false);
    ...
    private boolean check() {
        ...
        if (!((this.offHand || this.mainHand || this.switchMode.getValue() != Switch.BREAKSLOT || this.switching) && DamageUtill.canBreakWeakness(AutoCrystal.mc.player) && this.switchTimer.passedMs(this.switchCooldown.getValue().intValue()))) {
            this.renderPos = null;
            target = null;
            this.rotating = false;
            return false;
    ...
    private void mapCrystals() {
        ...
        for (Entity entity : AutoCrystal.mc.world.loadedEntityList) {
            ...
            float selfDamage = -1.0f;
            if (DamageUtill.canTakeDamage(this.suicide.getValue())) {
                selfDamage = DamageUtill.calculateDamage(entity, AutoCrystal.mc.player);
            }
            if ((double) selfDamage + 0.5 < (double) EntityUtill.getHealth(AutoCrystal.mc.player) && selfDamage <= this.maxSelfBreak.getValue().floatValue()) {
                ...
                for (EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                    float damage;
                    if (!(player.getDistanceSq(entity) <= MathUtill.square(this.range.getValue().floatValue())))
                        continue;
                    if (EntityUtill.isValid(player, this.range.getValue().floatValue() + this.breakRange.getValue().floatValue())) {
                        if (this.antiNaked.getValue().booleanValue() && DamageUtill.isNaked(player) || !((damage = DamageUtill.calculateDamage(entity, player)) > selfDamage || damage > this.minDamage.getValue().floatValue() && !DamageUtill.canTakeDamage(this.suicide.getValue())) && !(damage > EntityUtill.getHealth(player)))
                            continue;
                        if (damage > maxDamage) {
                            maxDamage = damage;
                            maxCrystal = entity;
                    ...
                    if (this.antiFriendPop.getValue() != AntiFriendPop.BREAK && this.antiFriendPop.getValue() != AntiFriendPop.ALL || !OyVey.friendManager.isFriend(player.getName()) || !((double) (damage = DamageUtill.calculateDamage(entity, player)) > (double) EntityUtill.getHealth(player) + 0.5))
                        continue;
        ...
        if (this.webAttack.getValue().booleanValue() && this.webPos != null) {
            if (AutoCrystal.mc.player.getDistanceSq(this.webPos.up()) > MathUtill.square(this.breakRange.getValue().floatValue())) {
                this.webPos = null;
            } else {
    ...
    private void calculateDamage(EntityPlayer targettedPlayer) {
        ...
        block0:
        for (BlockPos pos : BlockUtill.possiblePlacePositions(this.placeRange.getValue().floatValue(), this.antiSurround.getValue(), this.oneDot15.getValue())) {
            if (!BlockUtill.rayTracePlaceCheck(pos, (this.raytrace.getValue() == Raytrace.PLACE || this.raytrace.getValue() == Raytrace.FULL) && AutoCrystal.mc.player.getDistanceSq(pos) > MathUtill.square(this.placetrace.getValue().floatValue()), 1.0f))
                continue;
            float selfDamage = -1.0f;
            if (DamageUtill.canTakeDamage(this.suicide.getValue())) {
                selfDamage = DamageUtill.calculateDamage(pos, AutoCrystal.mc.player);
            }
            if (!((double) selfDamage + 0.5 < (double) EntityUtill.getHealth(AutoCrystal.mc.player)) || !(selfDamage <= this.maxSelfPlace.getValue().floatValue()))
                continue;
            if (targettedPlayer != null) {
                float playerDamage = DamageUtill.calculateDamage(pos, targettedPlayer);
                if (this.calcEvenIfNoDamage.getValue().booleanValue() && (this.antiFriendPop.getValue() == AntiFriendPop.ALL || this.antiFriendPop.getValue() == AntiFriendPop.PLACE)) {
                    ...
                    for (EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        float friendDamage;
                        if (friend == null || AutoCrystal.mc.player.equals(friend) || friend.getDistanceSq(pos) > MathUtill.square(this.range.getValue().floatValue() + this.placeRange.getValue().floatValue()) || !OyVey.friendManager.isFriend(friend) || !((double) (friendDamage = DamageUtill.calculateDamage(pos, friend)) > (double) EntityUtill.getHealth(friend) + 0.5))
                            continue;
                        friendPop = true;
                        break;
                    }
                ...
                if (this.foundDoublePop || !(playerDamage > maxDamage) && (!this.extraSelfCalc.getValue().booleanValue() || !(playerDamage >= maxDamage) || !(selfDamage < maxSelfDamage)) || !(playerDamage > selfDamage || playerDamage > this.minDamage.getValue().floatValue() && !DamageUtill.canTakeDamage(this.suicide.getValue())) && !(playerDamage > EntityUtill.getHealth(targettedPlayer)))
                    continue;
                maxDamage = playerDamage;
                currentTarget = targettedPlayer;
                currentPos = pos;
            ...
            for (EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                float friendDamage;
                if (EntityUtill.isValid(player, this.placeRange.getValue().floatValue() + this.range.getValue().floatValue())) {
                    if (this.antiNaked.getValue().booleanValue() && DamageUtill.isNaked(player)) continue;
                    float playerDamage = DamageUtill.calculateDamage(pos, player);
                    ...
                    if (this.foundDoublePop || !(playerDamage > maxDamage) && (!this.extraSelfCalc.getValue().booleanValue() || !(playerDamage >= maxDamage) || !(selfDamage < maxSelfDamage)) || !(playerDamage > selfDamage || playerDamage > this.minDamage.getValue().floatValue() && !DamageUtill.canTakeDamage(this.suicide.getValue())) && !(playerDamage > EntityUtill.getHealth(player)))
                        continue;
                    maxDamage = playerDamage;
                    currentTarget = player;
                    currentPos = pos;
                ...
                if (this.antiFriendPop.getValue() != AntiFriendPop.ALL && this.antiFriendPop.getValue() != AntiFriendPop.PLACE || player == null || !(player.getDistanceSq(pos) <= MathUtill.square(this.range.getValue().floatValue() + this.placeRange.getValue().floatValue())) || !OyVey.friendManager.isFriend(player) || !((double) (friendDamage = DamageUtill.calculateDamage(pos, player)) > (double) EntityUtill.getHealth(player) + 0.5))
                    continue;
    ...
    private EntityPlayer getTarget(boolean unsafe) {
        ...
        EntityPlayer currentTarget = null;
        for (EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
            if (EntityUtill.isntValid(player, this.placeRange.getValue().floatValue() + this.range.getValue().floatValue()) || this.antiNaked.getValue().booleanValue() && DamageUtill.isNaked(player) || unsafe && EntityUtill.isSafe(player))
                continue;
        ...
        if (this.predictPos.getValue().booleanValue() && currentTarget != null) {
            ...
            newTarget.setHealth(EntityUtill.getHealth(currentTarget));
    ...
    private void rotateTo(Entity entity) {
        switch (this.rotate.getValue()) {
            ...
            case BREAK:
            case ALL: {
                float[] angle = MathUtill.calcAngle(AutoCrystal.mc.player.getPositionEyes(mc.getRenderPartialTicks()), entity.getPositionVector());
    ...
    private void rotateToPos(BlockPos pos) {
        switch (this.rotate.getValue()) {
            ...
            case PLACE:
            case ALL: {
                float[] angle = MathUtill.calcAngle(AutoCrystal.mc.player.getPositionEyes(mc.getRenderPartialTicks()), new Vec3d((float) pos.getX() + 0.5f, (float) pos.getY() - 0.5f, (float) pos.getZ() + 0.5f));
                if (this.eventMode.getValue() == 2 && this.threadMode.getValue() == ThreadMode.NONE) {
                    OyVey.rotationManager.setPlayerRotations(angle[0], angle[1]);
                    break;
                }
    ...
    private boolean isDoublePoppable(EntityPlayer player, float damage) {
        float health;
        if (this.doublePop.getValue().booleanValue() && (double) (health = EntityUtill.getHealth(player)) <= this.popHealth.getValue() && (double) damage > (double) health + 0.5 && damage <= this.popDamage.getValue().floatValue()) {
    ...
    private boolean isValid(Entity entity) {
        return entity != null && AutoCrystal.mc.player.getDistanceSq(entity) <= MathUtill.square(this.breakRange.getValue().floatValue()) && (this.raytrace.getValue() == Raytrace.NONE || this.raytrace.getValue() == Raytrace.PLACE || AutoCrystal.mc.player.canEntityBeSeen(entity) || !AutoCrystal.mc.player.canEntityBeSeen(entity) && AutoCrystal.mc.player.getDistanceSq(entity) <= MathUtill.square(this.breaktrace.getValue().floatValue()));
    }
