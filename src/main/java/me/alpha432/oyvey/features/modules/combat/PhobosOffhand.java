...
public class PhobosOffhand extends Module {
    ...
    @Override
    public void onTick() {
        if (PhobosOffhand.nullCheck() || this.updates.getValue() == 1) {
            return;
        }
        this.doOffhand();
    }
    ...
    public void onUpdateWalkingPlayer(ProcessRightClickBlockEvent event) {
        if (this.noOffhandGC.getValue().booleanValue() && event.hand == EnumHand.MAIN_HAND && event.stack.getItem() == Items.END_CRYSTAL && Offhand.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE && Offhand.mc.objectMouseOver != null && event.pos == Offhand.mc.objectMouseOver.getBlockPos()) {
            ...
            PhobosOffhand.mc.player.setActiveHand(EnumHand.OFF_HAND);
            PhobosOffhand.mc.playerController.processRightClick((EntityPlayer)Offhand.mc.player, (World)Offhand.mc.world, EnumHand.OFF_HAND);
        }
    }
    ...
    public void onUpdate() {
        if (this.noOffhandGC.getValue().booleanValue() && this.retardOGC.getValue().booleanValue()) {
            if (this.timer.passedMs(this.timeout.getValue().intValue())) {
                if (PhobosOffhand.mc.player != null && Offhand.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE && Offhand.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL && Mouse.isButtonDown((int)1)) {
                    PhobosOffhand.mc.player.setActiveHand(EnumHand.OFF_HAND);
                    PhobosOffhand.mc.gameSettings.keyBindUseItem.pressed = Mouse.isButtonDown((int)1);
                }
            } else if (PhobosOffhand.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE && PhobosOffhand.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL) {
                PhobosOffhand.mc.gameSettings.keyBindUseItem.pressed = false;
            }
        }
        if (PhobosOffhand.nullCheck() || this.updates.getValue() == 2) {
            return;
        }
        this.doOffhand();
