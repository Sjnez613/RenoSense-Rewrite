...
public class Auto32k
    ...
    private void rotateToPos(BlockPos pos, Vec3d vec3d) {
        float[] angle = vec3d == null ? MathUtil.calcAngle(Auto32k.mc.player.getPositionEyes(Util.mc.getRenderPartialTicks()), new Vec3d((float) pos.getX() + 0.5f, (float) pos.getY() - 0.5f, (float) pos.getZ() + 0.5f)) : RotationUtil.getLegitRotations(vec3d);
        this.yaw = angle[0];
        this.pitch = angle[1];
        this.spoof = true;
    }
