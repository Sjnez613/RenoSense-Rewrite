...
public class Cosmetics
    ...
    public void onRenderPlayer(RenderPlayerEvent.Post event) {
        if (!RenoSense.cosmeticsManager.hasCosmetics(event.getEntityPlayer()) || EntityUtil.isFakePlayer(event.getEntityPlayer())) {
            return;
        }
        for (ModelBase model : RenoSense.cosmeticsManager.getRenderModels(event.getEntityPlayer())) {
            ...
            if (model instanceof TopHatModel) {
                ...
                Util.mc.getTextureManager().bindTexture(this.santaHatTexture);
                this.santaHatModel.render(event.getEntity(), 0.0f, 0.0f, -0.1f, 0.0f, 0.0f, 0.0625f);
                Util.mc.getTextureManager().deleteTexture(this.santaHatTexture);
            } else if (model instanceof ModelCloutGoggles) {
                Util.mc.getTextureManager().bindTexture(this.cloutGoggleTexture);
                this.cloutGoggles.render(event.getEntity(), 0.0f, 0.0f, -0.1f, 0.0f, 0.0f, 0.0625f);
                Util.mc.getTextureManager().deleteTexture(this.cloutGoggleTexture);
            } else if (model instanceof ModelSquidFlag) {
                Util.mc.getTextureManager().bindTexture(this.squidTexture);
                this.flag.render(event.getEntity(), 0.0f, 0.0f, -0.1f, 0.0f, 0.0f, 0.0625f);
                Util.mc.getTextureManager().deleteTexture(this.squidTexture);
            } else if (model instanceof ModelSquidLauncher) {
                Util.mc.getTextureManager().bindTexture(this.squidLauncherTexture);
                this.squidLauncher.render(event.getEntity(), 0.0f, 0.0f, -0.1f, 0.0f, 0.0f, 0.0325f);
                Util.mc.getTextureManager().deleteTexture(this.squidLauncherTexture);
            }
