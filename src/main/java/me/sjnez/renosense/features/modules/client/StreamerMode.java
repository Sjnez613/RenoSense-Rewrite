...
public class StreamerMode
    ...
    public void onUpdate() {
        if (this.window != null) {
            ...
            drawInfo.add("TPS: " + RenoSense.serverManager.getTPS());
            drawInfo.add("Ping: " + RenoSense.serverManager.getPing() + "ms");
            drawInfo.add("Speed: " + RenoSense.speedManager.getSpeedKpH() + "km/h");
            ...
            String text = RenoSense.rotationManager.getDirection4D(false);
            drawInfo.add("");
            drawInfo.add(text);
            drawInfo.add(coordinates);
            drawInfo.add("");
            for (Module module : RenoSense.moduleManager.sortedModules) {
                String moduleName = TextUtil.stripColor(module.getFullArrayString());
                drawInfo.add(moduleName);
            }
            drawInfo.add("");
            for (PotionEffect effect : RenoSense.potionManager.getOwnPotions()) {
                String potionText = TextUtil.stripColor(RenoSense.potionManager.getColoredPotionString(effect));
