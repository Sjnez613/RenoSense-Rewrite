...
public class Skeleton
    ...
    public void onRender3D(Render3DEvent event) {
        ...
        for (EntityPlayer player : Skeleton.mc.world.playerEntities) {
            if (player == null || player == Util.mc.getRenderViewEntity() || !player.isEntityAlive() || player.isPlayerSleeping() || player.isInvisible() && !this.invisibles.getValue().booleanValue() || this.rotationList.get(player) == null || !(Skeleton.mc.player.getDistanceSq(player) < 2500.0))
                continue;
    ...
    private void renderSkeleton(EntityPlayer player, float[][] rotations, Color color) {
        ...
        Vec3d interp = EntityUtil.getInterpolatedRenderPos(player, Util.mc.getRenderPartialTicks());
        double pX = interp.x;
        double pY = interp.y;
        double pZ = interp.z;
